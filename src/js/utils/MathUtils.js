...
class MathUtils {
   ...
   * @return {Array} The resultant array (same size as input)
   */
  static convolve(
    matrix,
    kernel = GameConstants.NEIGHBOURHOODS.MOORES.kernel,
    evaluationFunction = (value) => value,
  ) {
    const maxSize = matrix.length;
    const kernelSize = kernel.length;

    // Initialise new matrix with padding
    const newMatrix = Array(maxSize + 2)
      .fill(0)
      .map(() => Array(maxSize + 2).fill(0));
