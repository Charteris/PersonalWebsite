...
class Resume extends React.Component {
  ...
  cycleSection(offset = 1) {
    let newIndex =
      (this.state.titleIndex + offset) % ResumeConstants.TITLES.length;
    if (newIndex < 0) {
  ...
  renderSection() {
        ...
        <div className="main-section">
          <div className="info-section">
            {artefact.Images.map((image) => (
              <img className="image-field" alt="" src={image} key={image} />
            ))}
            {artefact.Link ? (
          ...
          <div className="text-section">
            {artefact.Description.map((text) => (
              <div className="text-field" key={text}>
                {text}
              </div>
            ))}
          </div>
    ...
    return (
      <div className="main-section">
        <div className="text-section">
          {ResumeConstants.ABOUT_ME.map((paragraph, index) => (
        ...
        <div className="info-section">
          ...
          <div className="general-info-section">
            {Object.entries(ResumeConstants.GENERAL_INFO).map(
              ([field, value]) => (
                <div
          ...
          <div className="links">
            {Object.entries(ResumeConstants.SOCIALS).map(([field, link]) => (
              <a className="link" href={link} rel="noreferrer" key={field}>
